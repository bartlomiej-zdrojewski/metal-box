document.addEventListener('DOMContentLoaded', function (event) {
    ...
    function setSpan(id, text, isError) {
        ...
        if (text !== undefined) {
            let spanHtml = "<span";
            if (id === BUTTON_LOGIN_ID || isError === true) {
                spanHtml += " class=\"";
                if (id === BUTTON_LOGIN_ID) {
                    spanHtml += "status ";
                }
                if (isError === true) {
                    spanHtml += "error "
    ...
    function login() {
        ...
        fetch(url, params)
            ...
            .then(response => {
                if (response.success) {
                    setSpan(BUTTON_LOGIN_ID, "Logowanie zakończyło się pomyślnie.<br />Przekierowywanie...");
                    window.location.replace(response.redirect_url);
                } else {
                    console.log("Login error: " + response.error_message);
                    setSpan(BUTTON_LOGIN_ID, "Logowanie nie powiodło się.<br />Nieprawidłowy login lub hasło.", true);
                    isLogging = false;
                }
            })
            .catch(error => {
