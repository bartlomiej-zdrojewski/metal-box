...
class Api:
    ...
    def fetchCourierPackageList(self, login):
        if not self.doesUserExist(login):
            abort(500,
                  "Could not fetch package list. User does not exists "
                  "(login: {}).".format(login))
        if not self.isUserCourier(login):
            abort(500,
                  "Could not fetch package list. User is not a courier "
                  "(login: {}).".format(login))
        package_list = []
        courier_id = self.getUserIdFromLogin(login)
        for package_id in self.db.hkeys(PACKAGE_ID_TO_COURIER_ID_MAP):
            if courier_id == self.db.hget(PACKAGE_ID_TO_COURIER_ID_MAP,
                                          package_id):
